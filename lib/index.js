...
if (process.platform === "win32") {
    ...
    chromedriverVersion = '\\chromedriver_2.34.exe';
    ...
    chromedriverVersion = '\\chromedriver2_34';
...
const start = () => {
    ...
    http.get({ host: '127.0.0.1', port: port, path: '/wd/hub/status' }, (res) => {
        ...
        res.on('end', () => {
            try {
                ...
                console.log(`Selenium server allready running on localhost port ${port} 
...
`);
        ...
        if (e.message === `connect ECONNREFUSED 127.0.0.1:${port}`) {
            console.error(`No selenium server currently running on localhost:${port} \nStarting new selenium server instance...`);
            const selenium = spawn('Java', [pathToEdge, pathToChrome, pathToFirefox, pathToPhantom, `-jar`, pathToJar, '-port', port]);
